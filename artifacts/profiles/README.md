# 历史性能基线：p2 → wuwei-ws 运营商 IPv6 / 自适应 FEC 实测

> **保留决定（2026-08-22）**：本目录中的 JSON、SVG、report 和客户端日志是已筛选的历史性能证据，不是当前版本的性能承诺。它们依赖当时的远端主机、运营商路径、内核、配置和二进制，仓库内没有足以逐字节重建这些 WAN 运行的命令/配置/版本清单，因此本轮不删除或迁移。当前性能验证流程见[性能验证](../../docs/性能验证.md)；新的原始采集应先写入 `target/`，只在补齐场景、命令、版本和采样完整性说明后再提交精选基线。
