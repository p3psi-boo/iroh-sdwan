...
- FlowRouter：按方向性流键稳定分片到最多 8 个单写者 shard，并在可达候选首跳中选择一个路径。
...
目的所有者直接连接且健康时，FlowRouter 选择直接 peer。连接存在但丢包或健康度越过阈值时，直连仍保留为候选，同时开放来自有效 Presence 或固定配置中 `transit_enabled = true` 的已连接中转候选；因此“connected”不会再永久压制健康的绕行路径。
