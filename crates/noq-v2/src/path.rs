...
impl Path {
    ...
    /// Send one congestion-window-exempt PING + PATH_CHALLENGE on a warm path.
    pub fn probe_liveness(&self) -> Result<(), ClosedPath> {
        let mut state = self.conn.lock_and_wake("probe path liveness");
        state.inner.probe_path_liveness(self.id())
    }
}
