...
impl Connecting {
    pub(crate) fn new(
        ...
        let (datagram_batch_tx, datagram_batch_rx) = mpsc::channel(8);
        let (priority_datagram_batch_tx, priority_datagram_batch_rx) = mpsc::channel(8);

        let max_datagram_size = conn.datagrams().max_size().unwrap_or(0);
        let datagram_buffer_capacity = conn.datagrams().send_buffer_size();
        let conn = ConnectionRef(Arc::new(Arc::new(ConnectionInner {
            state: Mutex::new(State::new(
                ...
                runtime.clone(),
                datagram_batch_rx,
                priority_datagram_batch_rx,
            )),
            datagram_batch_tx,
            priority_datagram_batch_tx,
            shared: Shared {
                max_datagram_size: AtomicUsize::new(max_datagram_size),
                datagram_buffer_capacity,
                datagram_buffer_limit: AtomicUsize::new(datagram_buffer_capacity),
                ..Shared::default()
            },
        })));
...
impl Future for ConnectionDriver {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        // Apply protocol events (including DATAGRAM buffer availability)
        // before admitting producer-mailbox batches.
        conn.forward_app_events(&self.0.shared);
        let mut keep_going = conn.drive_buffered_datagrams(cx);
        keep_going |= conn.drive_transmit(cx)?;
        // `drive_buffered_datagrams` runs before packet construction. Packet
        // construction can free native noq-proto DATAGRAM budget in this same
        // poll, after the mailbox head already observed `Blocked`. The
        // protocol's DatagramsUnblocked event wakes public send futures, but
        // this driver-owned mailbox has no external waiter. Re-poll exactly
        // when its head now fits; otherwise sleep for the normal ACK/timer
        // wakeup. This closes the internal lost-wakeup window without a spin.
        if let Some(next) = conn.pending_datagram_batch.front()
            && next.len() <= conn.inner.datagrams().send_buffer_space()
        {
            keep_going = true;
        }
        ...
        conn.forward_app_events(&self.0.shared);
        self.0.shared.max_datagram_size.store(
            conn.inner.datagrams().max_size().unwrap_or(0),
            Ordering::Relaxed,
        );
...
impl Connection {
    ...
    /// Wait for the first application datagram, then drain the remainder of
    /// the already-buffered burst under the same connection-state lock.
    pub fn read_datagram_batch(&self, maximum: usize) -> ReadDatagramBatch<'_> {
        ReadDatagramBatch {
            conn: &self.0,
            notify: self.0.shared.datagram_received.notified(),
    ...
    /// Drain one already-buffered application datagram without registering a
    /// wakeup. Callers await [`Self::read_datagram`] once, then use this method
    /// to coalesce the remainder of the current receive burst behind a single
    /// connection-state lock/wakeup boundary.
    pub fn try_read_datagram(&self) -> Result<Option<Bytes>, ConnectionError> {
        let mut state = self.0.lock_without_waking("Connection::try_read_datagram");
        if let Some(datagram) = state.inner.datagrams().recv() {
            Ok(Some(datagram))
    ...
    /// Drain up to `maximum` buffered datagrams while holding the connection
    /// state once. The returned vector is empty when the current burst has
    /// been drained; it never waits for a future packet.
    pub fn try_read_datagram_batch(&self, maximum: usize) -> Result<Vec<Bytes>, ConnectionError> {
        if maximum == 0 {
            return Ok(Vec::new());
        }
        let mut state = self
            .0
            .lock_without_waking("Connection::try_read_datagram_batch");
        let mut datagrams = Vec::with_capacity(maximum);
        while datagrams.len() < maximum {
    ...
    /// Transmit a bounded batch of unreliable, unordered application datagrams.
    ///
    /// This has the same backpressure semantics as [`send_datagram_wait()`], but
    /// admits all currently-unblocked datagrams while holding the connection
    /// state lock once. It is intended for application-level packet trains where
    /// several adjacent datagrams are already scheduled as one bounded quantum.
    ///
    /// If the send buffer fills, the returned future waits and resumes with the
    /// first uncommitted datagram. An error can therefore be returned after an
    /// earlier prefix of the batch has been committed to QUIC.
    ///
    /// [`send_datagram_wait()`]: Connection::send_datagram_wait
    pub fn send_datagram_batch_wait(&self, datagrams: Vec<Bytes>) -> SendDatagramBatch<'_> {
        SendDatagramBatch {
            conn: &self.0,
            datagrams: datagrams.into_iter().map(Into::into).collect(),
            notify: self.0.shared.datagrams_unblocked.notified(),
        }
    }

    /// Submit one bounded DATAGRAM batch to the connection driver without
    /// taking the protocol-state mutex on the producer task.
    ///
    /// The channel is deliberately shallow: awaiting capacity preserves
    /// backpressure while avoiding a shared-lock handoff for every Bulk
    /// quantum.
    pub async fn send_datagram_batch_buffered(
        &self,
        datagrams: Vec<Bytes>,
    ) -> Result<(), SendDatagramError> {
        self.send_datagram_payload_batch_buffered(datagrams.into_iter().map(Into::into).collect())
            .await
    }

    /// Submit a bounded batch whose DATAGRAM payloads may consist of two
    /// adjacent immutable ranges, avoiding application-side coalescing.
    pub async fn send_datagram_payload_batch_buffered(
        &self,
        datagrams: Vec<proto::DatagramPayload>,
    ) -> Result<(), SendDatagramError> {
        if datagrams.is_empty() {
            return Ok(());
        }
        let Some(maximum) = self.max_datagram_size() else {
            return Err(SendDatagramError::UnsupportedByPeer);
        };
        if datagrams.iter().any(|datagram| datagram.len() > maximum) {
            return Err(SendDatagramError::TooLarge);
        }
        self.0.datagram_batch_tx.send(datagrams).await.map_err(|_| {
            let error = self
                .0
                .lock_without_waking("send_datagram_batch_buffered::closed")
                .error
                .clone()
    ...
    /// Submit a strict-priority DATAGRAM batch to the connection driver.
    /// Priority batches bypass staged ordinary batches and are inserted ahead
    /// of ordinary DATAGRAMs already accepted by the QUIC protocol buffer.
    /// Ordering inside each priority batch remains FIFO.
    pub async fn send_datagram_batch_buffered_priority(
        &self,
        datagrams: Vec<Bytes>,
    ) -> Result<(), SendDatagramError> {
        self.send_datagram_payload_batch_buffered_priority(
            datagrams.into_iter().map(Into::into).collect(),
        )
        .await
    }

    /// Submit a strict-priority batch of contiguous or split DATAGRAM
    /// payloads through the connection-driver mailbox.
    pub async fn send_datagram_payload_batch_buffered_priority(
        &self,
        datagrams: Vec<proto::DatagramPayload>,
    ) -> Result<(), SendDatagramError> {
        if datagrams.is_empty() {
        ...
        self.0
            ...
            .map_err(|_| {
                let error = self
                    .0
                    .lock_without_waking("send_datagram_batch_buffered_priority::closed")
                    .error
                    .clone()
                    .unwrap_or(ConnectionError::LocallyClosed);
                SendDatagramError::ConnectionLost(error)
    ...
    pub fn max_datagram_size(&self) -> Option<usize> {
        match self.0.shared.max_datagram_size.load(Ordering::Relaxed) {
            0 => None,
            size => Some(size),
        }
    }
    ...
    /// Set the connection-local QUIC DATAGRAM byte limit. The immutable
    /// transport value remains the hard maximum; shrinking retains accepted
    /// DATAGRAMs and uses noq-proto's native blocked/unblocked event path.
    pub fn set_datagram_send_buffer_limit(&self, value: usize) -> usize {
        let capacity = self.0.shared.datagram_buffer_capacity;
        if capacity == 0 {
            return 0;
        }
        let minimum = self.max_datagram_size().unwrap_or(1).min(capacity);
        let effective = value.clamp(minimum, capacity);
        let mut state = self.0.lock_and_wake("set_datagram_send_buffer_limit");
        state.inner.datagrams().set_send_buffer_size(effective);
        state.forward_app_events(&self.0.shared);
        self.0
            .shared
            .datagram_buffer_limit
            .store(effective, Ordering::Release);
        effective
    }

    /// Current connection-local QUIC DATAGRAM byte limit. This snapshot is
    /// updated by [`Self::set_datagram_send_buffer_limit`] after the protocol
    /// state has accepted the same value, so admission checks never contend
    /// with the connection driver merely to reread stable tuning state.
    pub fn datagram_send_buffer_limit(&self) -> usize {
        self.0.shared.datagram_buffer_limit.load(Ordering::Acquire)
    }
    ...
    /// Runtime tuning handle of the path congestion controller, when supported.
    pub fn congestion_tunables(&self, path_id: PathId) -> Option<Arc<dyn Any + Send + Sync>> {
        self.0
            .lock_without_waking("congestion_tunables")
            .inner
            .congestion_tunables(path_id)
    }
...
pin_project! {
    /// Future produced by [`Connection::read_datagram_batch`].
    pub struct ReadDatagramBatch<'a> {
        conn: &'a ConnectionRef,
        #[pin]
        notify: Notified<'a>,
        maximum: usize,
...
impl Future for ReadDatagramBatch<'_> {
    type Output = Result<Vec<Bytes>, ConnectionError>;

    fn poll(self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Self::Output> {
        let mut this = self.project();
        let mut state = this.conn.lock_without_waking("ReadDatagramBatch::poll");
        // The lock covers both readiness and the complete burst drain. This
        // avoids handing the connection mutex back to the driver between the
        // first DATAGRAM and its already-buffered siblings.
        if let Some(first) = state.inner.datagrams().recv() {
            let mut datagrams = Vec::with_capacity(*this.maximum);
            datagrams.push(first);
            while datagrams.len() < *this.maximum {
                let Some(datagram) = state.inner.datagrams().recv() else {
                    break;
                };
                datagrams.push(datagram);
...
pin_project! {
    ...
    pub struct SendDatagram<'a> {
        conn: &'a ConnectionRef,
        data: Option<proto::DatagramPayload>,
        #[pin]
        notify: Notified<'a>,
    }
}
...
impl Future for SendDatagram<'_> {
    ...
    fn poll(self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        match state
            .inner
            .datagrams()
            .send_payload(this.data.take().unwrap(), false)
        {
            Ok(()) => Poll::Ready(Ok(())),
...
pin_project! {
    /// Future produced by [`Connection::send_datagram_batch_wait`].
    pub struct SendDatagramBatch<'a> {
        conn: &'a ConnectionRef,
        datagrams: VecDeque<proto::DatagramPayload>,
        #[pin]
        notify: Notified<'a>,
    }
}

impl Future for SendDatagramBatch<'_> {
    type Output = Result<(), SendDatagramError>;

    fn poll(self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<Self::Output> {
        let mut this = self.project();
        let mut state = this.conn.lock_and_wake("SendDatagramBatch::poll");
        if let Some(ref error) = state.error {
            return Poll::Ready(Err(SendDatagramError::ConnectionLost(error.clone())));
        }

        use proto::SendDatagramError::*;
        loop {
            let Some(data) = this.datagrams.pop_front() else {
                return Poll::Ready(Ok(()));
            };
            match state.inner.datagrams().send_payload(data, false) {
                Ok(()) => {}
                Err(Blocked(data)) => {
                    this.datagrams.push_front(data);
                    loop {
                        match this.notify.as_mut().poll(ctx) {
                            Poll::Pending => return Poll::Pending,
                            // Spurious wakeup, get a new future while the state lock
                            // ensures that readiness cannot be missed.
                            Poll::Ready(()) => this
                                .notify
                                .set(this.conn.shared.datagrams_unblocked.notified()),
                        }
                ...
                Err(UnsupportedByPeer) => {
                    return Poll::Ready(Err(SendDatagramError::UnsupportedByPeer));
                }
                Err(Disabled) => return Poll::Ready(Err(SendDatagramError::Disabled)),
                Err(TooLarge) => return Poll::Ready(Err(SendDatagramError::TooLarge)),
            }
        }
    }
}
...
pub(crate) struct ConnectionInner {
    ...
    state: Mutex<State>,
    datagram_batch_tx: mpsc::Sender<Vec<proto::DatagramPayload>>,
    priority_datagram_batch_tx: mpsc::Sender<Vec<proto::DatagramPayload>>,
...
pub(crate) struct Shared {
    ...
    /// Lock-free snapshot of the current QUIC DATAGRAM payload limit. Zero
    /// represents unsupported/disabled, since a usable DATAGRAM is never empty.
    max_datagram_size: AtomicUsize,
    /// Immutable hard ceiling configured before the connection was created.
    datagram_buffer_capacity: usize,
    /// Lock-free snapshot of the live connection-local DATAGRAM byte limit.
    datagram_buffer_limit: AtomicUsize,
}
...
pub(crate) struct State {
    ...
    datagram_batch_rx: mpsc::Receiver<Vec<proto::DatagramPayload>>,
    pending_datagram_batch: VecDeque<proto::DatagramPayload>,
    priority_datagram_batch_rx: mpsc::Receiver<Vec<proto::DatagramPayload>>,
    pending_priority_datagram_batch: VecDeque<proto::DatagramPayload>,
...
impl State {
    ...
    fn new(
        ...
        Self {
            ...
            datagram_batch_rx,
            pending_datagram_batch: VecDeque::new(),
            priority_datagram_batch_rx,
            pending_priority_datagram_batch: VecDeque::new(),
    ...
    /// Move staged application DATAGRAMs into the QUIC protocol buffer while
    /// the connection driver already owns the state. Returns true when the
    /// hard work bound was reached and the driver should poll again.
    fn drive_buffered_datagrams(&mut self, cx: &mut Context<'_>) -> bool {
        const MAX_ADMISSIONS_PER_POLL: usize = 64;
        let mut admitted = 0_usize;
        loop {
            // Poll the strict lane on every iteration, even while an ordinary
            // batch is partially staged. This both registers the driver's
            // waker and lets a newly arrived interactive batch leapfrog Bulk.
            if self.pending_priority_datagram_batch.is_empty()
                && let Poll::Ready(Some(batch)) = self.priority_datagram_batch_rx.poll_recv(cx)
            {
                self.pending_priority_datagram_batch = batch.into();
            }
            if let Some(data) = self.pending_priority_datagram_batch.pop_front() {
                use proto::SendDatagramError::*;
                match self.inner.datagrams().send_payload_priority(data) {
                    Ok(()) => {
                        admitted += 1;
                        if admitted >= MAX_ADMISSIONS_PER_POLL {
                            return true;
                        }
                        continue;
                    }
                    Err(Blocked(data)) => {
                        self.pending_priority_datagram_batch.push_front(data);
                        return false;
                    }
                    Err(TooLarge | UnsupportedByPeer | Disabled) => continue,
                }
            }
            if self.pending_datagram_batch.is_empty() {
                match self.datagram_batch_rx.poll_recv(cx) {
                    Poll::Ready(Some(batch)) => self.pending_datagram_batch = batch.into(),
                    Poll::Ready(None) | Poll::Pending => return false,
                }
            }
            let data = self
                .pending_datagram_batch
                .pop_front()
                .expect("non-empty buffered DATAGRAM batch");
            use proto::SendDatagramError::*;
            match self.inner.datagrams().send_payload(data, false) {
                Ok(()) => {
                    admitted += 1;
                    if admitted >= MAX_ADMISSIONS_PER_POLL {
                        return true;
                ...
                Err(Blocked(data)) => {
                    self.pending_datagram_batch.push_front(data);
                    return false;
                }
                // DATAGRAM is lossy by contract. A PMTU shrink can race the
                // producer's atomic ceiling snapshot; drop this bounded
                // quantum instead of closing the authenticated session.
                Err(TooLarge | UnsupportedByPeer | Disabled) => {}
            }
        }
    }
    ...
    fn terminate(&mut self, reason: ConnectionError, shared: &Shared) {
        ...
        self.datagram_batch_rx.close();
        self.priority_datagram_batch_rx.close();
        self.pending_datagram_batch.clear();
        self.pending_priority_datagram_batch.clear();
        while self.datagram_batch_rx.try_recv().is_ok() {}
        while self.priority_datagram_batch_rx.try_recv().is_ok() {}
        if let Some(x) = self.on_handshake_data.take() {
            let _ = x.send(());
        }
        wake_all(&mut self.blocked_writers);
