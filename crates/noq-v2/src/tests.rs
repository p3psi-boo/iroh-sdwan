...
#[tokio::test]
async fn buffered_datagram_batch_drains_without_extra_wakeups() {
    let _guard = subscribe();
    let endpoint = endpoint();
    let (client, server) = tokio::join!(
        endpoint
    ...
    let first = client.read_datagram().await.unwrap();
    tokio::time::sleep(Duration::from_millis(10)).await;
    let mut received = vec![first[0]];
    received.extend(
        client
            .try_read_datagram_batch(16)
            .unwrap()
            .into_iter()
            .map(|datagram| datagram[0]),
    );
    received.sort_unstable();
    assert_eq!(received, (0_u8..8).collect::<Vec<_>>());
    assert!(client.try_read_datagram_batch(16).unwrap().is_empty());
}

#[tokio::test]
async fn native_dynamic_datagram_limit_sustains_many_buffer_turns() {
    let _guard = subscribe();
    let endpoint = endpoint();
    let (client, server) = tokio::join!(
        endpoint
    ...
    let client = client.unwrap();
    let server = server.unwrap();
    let capacity = server.datagram_send_buffer_limit();
    let maximum = server.max_datagram_size().unwrap();
    let limit = maximum.saturating_mul(4).min(capacity);
    assert_eq!(server.set_datagram_send_buffer_limit(limit), limit);
    assert_eq!(server.datagram_send_buffer_limit(), limit);
    assert_eq!(client.datagram_send_buffer_limit(), capacity);

    let count = 4096_u32;
    let datagrams = (0..count)
        .map(|sequence| {
            let mut bytes = vec![0_u8; maximum.min(1024)];
            bytes[..4].copy_from_slice(&sequence.to_be_bytes());
            Bytes::from(bytes)
        })
        .collect::<Vec<_>>();
    tokio::time::timeout(Duration::from_secs(5), async {
        let send = server.send_datagram_batch_buffered(datagrams);
        let receive = async {
            let mut seen = vec![false; count as usize];
            let mut received = 0_u32;
            while received < count {
                for datagram in client.read_datagram_batch(64).await.unwrap() {
                    let sequence = u32::from_be_bytes(datagram[..4].try_into().unwrap()) as usize;
    ...
    // PMTU discovery can raise the live DATAGRAM ceiling while the sustained
    // transfer above is running. The minimum admission limit is one datagram
    // at the current ceiling, not the handshake-time value captured before
    // the transfer.
    let live_maximum = server.max_datagram_size().unwrap();
    assert_eq!(server.set_datagram_send_buffer_limit(1), live_maximum);
    assert_eq!(server.set_datagram_send_buffer_limit(usize::MAX), capacity);
}

#[tokio::test]
async fn awaited_datagram_batch_includes_buffered_burst() {
    let _guard = subscribe();
    let endpoint = endpoint();
    let (client, server) = tokio::join!(
        endpoint
...
#[tokio::test]
async fn driver_buffered_datagram_batch_preserves_bounded_backpressure() {
    let _guard = subscribe();
    let endpoint = endpoint();
    let (client, server) = tokio::join!(
        endpoint
    ...
    let expected = (0_u8..8)
        ...
        .collect::<Vec<_>>();
    server.send_datagram_batch_buffered(expected).await.unwrap();
    let mut received = Vec::new();
    while received.len() < 8 {
        received.extend(
            client
                .read_datagram_batch(8 - received.len())
                .await
                .unwrap()
                .into_iter()
                .map(|datagram| datagram[0]),
