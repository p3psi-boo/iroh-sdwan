...
/// A receive-batch accumulator that keeps the one-event case allocation-free.
enum PendingProtoEvents {
    One(proto::ConnectionEvent),
    Many(Vec<proto::ConnectionEvent>),
}

impl PendingProtoEvents {
    fn push(&mut self, event: proto::ConnectionEvent, batch_capacity_hint: usize) {
        match self {
            Self::One(_) => {
                let Self::One(first) = std::mem::replace(
                    self,
                    Self::Many(Vec::with_capacity(batch_capacity_hint.max(2))),
                ) else {
                    unreachable!();
                };
                let Self::Many(events) = self else {
                    unreachable!();
                };
                events.push(first);
                events.push(event);
            }
            Self::Many(events) => events.push(event),
        }
    }

    fn into_connection_event(self) -> ConnectionEvent {
        match self {
            Self::One(event) => ConnectionEvent::Proto(event),
            Self::Many(events) => ConnectionEvent::ProtoBatch(events),
        }
    }
}

struct PendingConnectionEvents {
    handle: ConnectionHandle,
    events: PendingProtoEvents,
}
        ...
        loop {
            match socket.poll_recv(cx, &mut iovs, &mut metas) {
                Poll::Ready(Ok(msgs)) => {
                    ...
                    // UDP GRO can expose many QUIC packets as strides inside
                    // one RecvMeta. Their count is already known before
                    // decoding, so reserve once when a connection receives
                    // its second event instead of geometrically reallocating
                    // and moving ConnectionEvent values throughout the batch.
                    let event_capacity_hint = metas
                        .iter()
                        .take(msgs)
                        .map(|meta| meta.len.div_ceil(meta.stride.max(1)))
                        .sum::<usize>()
                        .clamp(2, IO_LOOP_BOUND);
                    // A socket batch almost always targets one active QUIC
                    // connection. Keep that common case inline and allocate
                    // the fallback list only when multiple connections share
                    // this endpoint/socket batch.
                    let mut primary_events: Option<PendingConnectionEvents> = None;
                    let mut additional_events = Vec::<PendingConnectionEvents>::new();
                        ...
                        while !data.is_empty() {
                            ...
                            match endpoint.handle(
                                ...
                                Some(DatagramEvent::ConnectionEvent(handle, event)) => {
                                    ...
                                    match primary_events.as_mut() {
                                        Some(pending) if pending.handle == handle => {
                                            pending.events.push(event, event_capacity_hint);
                                        }
                                        None => {
                                            primary_events = Some(PendingConnectionEvents {
                                                handle,
                                                events: PendingProtoEvents::One(event),
                                            });
                                        }
                                        Some(_) => {
                                            if let Some(index) = additional_events
                                                .iter()
                                                .position(|pending| pending.handle == handle)
                                            {
                                                additional_events[index]
                                                    .events
                                                    .push(event, event_capacity_hint);
                                            } else {
                                                additional_events.push(PendingConnectionEvents {
                                                    handle,
                                                    events: PendingProtoEvents::One(event),
                                                });
                                            }
                    ...
                    for pending in primary_events.into_iter().chain(additional_events) {
                        // Ignore events for connections dropped before this
                        // receive batch completed, matching the old per-event
                        // send behavior.
                        if let Some(sender) = self.connections.senders.get_mut(&pending.handle) {
                            let _ = sender.send(pending.events.into_connection_event());
                        }
                    }
                }
                Poll::Pending => {
