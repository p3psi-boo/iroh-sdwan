...
pub use proto::{
    ...
    ConnectionIdGenerator, ConnectionStats, DatagramPayload, DecryptedInitial, Dir, EcnCodepoint,
    EndpointConfig, FourTuple, FrameStats, FrameType, IdleTimeout, InvalidCid, MtuDiscoveryConfig,
...
pub use crate::connection::{
    ...
    ReadDatagramBatch, SendDatagram, SendDatagramBatch, SendDatagramError, WeakConnectionHandle,
    ZeroRttAccepted,
};
...
enum ConnectionEvent {
    ...
    /// Protocol events produced by one UDP receive batch for one connection.
    ///
    /// Keeping the batch intact avoids one channel allocation and receiver
    /// wake attempt per packet on high-rate DATAGRAM paths.
