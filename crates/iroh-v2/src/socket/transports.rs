...
impl TransportsSender {
    // This is a per-packet hot path.  Keeping the span at INFO makes the
    // default subscriber allocate and format `network_path` for every QUIC
    // transmit, even though no event is emitted from the common send path.
    #[instrument(
        level = "trace",
        name = "poll_send",
        skip(self, cx, transmit),
        fields(len = transmit.contents.len())
    )]
