...
/// Consecutive QUIC probe timeouts that make a path ineligible while a healthier path is
/// available.  Four PTOs tolerate transient scheduler/ACK stalls under a saturated local
/// data plane while still detecting a hard black hole before the per-path idle timeout.
const DEGRADED_PTO_COUNT: u32 = 4;
...
impl PathSelector for BiasedRttPathSelector {
    fn select(&self, ctx: &PathSelectionContext<'_>) -> PathSelection {
        ...
        let mut best: Option<(PathSelectionData<'_>, (bool, TransportType, i128))> = None;
        let mut current_key: Option<(bool, TransportType, i128)> = None;
        ...
        for psd in ctx.paths() {
            ...
            let key = self.sort_key(network_path, rtt, stats.pto_count);
        ...
        let Some((best_psd, (best_degraded, best_tier, best_biased))) = best else {
            return selection;
        };

        // If we have no current path or no data for it, switch to the best.
        let Some((current_degraded, current_tier, current_biased)) = current_key else {
            selection.set(&best_psd);
            return selection;
        };

        if current_degraded != best_degraded {
            // A healthy warm path wins immediately over one missing consecutive ACKs.
            selection.set(&best_psd);
        } else if current_tier != best_tier {
...
mod tests {
    ...
    #[test]
    fn healthy_warm_path_replaces_degraded_current_immediately() {
        let fast_but_blackholed = v4(1);
        let slower_but_healthy = v4(2);

        // One PTO is treated as normal transient loss.
        let chosen = select_with_default(
            Some(&fast_but_blackholed),
            vec![
                psd_with_pto(&fast_but_blackholed, 5, 1),
                psd_with_pto(&slower_but_healthy, 30, 0),
            ],
        );
        assert_eq!(chosen, None);

        // Two consecutive PTOs move traffic even though the warm path has higher RTT.
        let chosen = select_with_default(
            Some(&fast_but_blackholed),
            vec![
                psd_with_pto(&fast_but_blackholed, 5, DEGRADED_PTO_COUNT),
                psd_with_pto(&slower_but_healthy, 30, 0),
            ],
        );
        assert_eq!(chosen.as_ref(), Some(&slower_but_healthy));
    }

    #[test]
    fn does_not_switch_when_every_path_is_degraded() {
        let current = v4(1);
        let candidate = v4(2);
        let chosen = select_with_default(
            Some(&current),
            vec![
                psd_with_pto(&current, 10, DEGRADED_PTO_COUNT),
                psd_with_pto(&candidate, 30, DEGRADED_PTO_COUNT + 1),
            ],
        );
        assert_eq!(chosen, None);
    }
