...
use crate::{
    ...
    endpoint::DirectAddr,
    socket::{
        MAX_MULTIPATH_PATHS, Metrics as SocketMetrics, RELAY_PATH_MAX_IDLE_TIMEOUT,
...
/// How often live QUIC path health is re-evaluated.
///
/// This is intentionally independent from [`UPGRADE_INTERVAL`]: waiting a minute is fine
/// for opportunistic hole-punch upgrades, but not for moving an established flow away from
/// a black-holed interface.  The signal itself comes from QUIC PTO state, so this timer does
/// not add packets to the wire.
const PATH_HEALTH_INTERVAL: Duration = Duration::from_millis(100);
...
struct State {
    ...
    /// Reopened paths that are proving stability before they may displace a working path.
    recovery_probation_until: FxHashMap<transports::Addr, Instant>,
    ...
    path_recovery_interval: Duration,
    path_recovery_probation: Duration,
}
...
impl RemoteStateActor {
    ...
    pub(super) fn new(
        ...
        Self {
            ...
            state: State {
                ...
                recovery_probation_until: FxHashMap::default(),
                address_lookup_stream: None,
                path_selector,
                path_recovery_interval,
                path_recovery_probation,
            },
        }
    }
    ...
    async fn run(
        ...
        let evaluate_path_health = time::interval(PATH_HEALTH_INTERVAL);
        n0_future::pin!(evaluate_path_health);

        let recover_paths = time::interval(self.state.path_recovery_interval);
        n0_future::pin!(recover_paths);
        ...
        loop {
            ...
            tokio::select! {
                ...
                // Health and recovery timers precede the data inbox deliberately.  This
                // select is biased and a saturated QUIC DATAGRAM producer keeps `recv()`
                // continuously ready; placing timers after it starved failover until the
                // full path idle timeout expired.
                _ = evaluate_path_health.tick() => {
                    self.select_path();
                }
                _ = recover_paths.tick() => {
                    self.reopen_known_paths();
                    self.probe_backup_paths();
                }
                msg = inbox.recv() => {
                    match msg {
    ...
    fn handle_msg_add_connection(
        ...
        if let Some(path) = conn.path(PathId::ZERO) {
            ...
            if path_remote.is_some() && conn.side().is_client() {
                // Keep independently addressed direct links warm as QUIC
                // Backup paths. Opening only the initially selected locator
                // made a second ISP/interface unusable until the entire
                // connection timed out, despite multipath being negotiated.
                // The transport limit bounds both memory and validation work.
                ...
                alternatives.truncate(MAX_MULTIPATH_PATHS as usize);
                for open_addr in alternatives {
                    if path_remote
                        .as_ref()
                        .is_some_and(|current| current.remote() == open_addr.remote())
                    {
                        continue;
                    }
                    self.state
    ...
    /// Keep validated alternatives observable even when their ordinary keepalives are
    /// congestion-blocked after an outage. One probe per recovery tick is enough for
    /// health selection and remains separate from application scheduling.
    fn probe_backup_paths(&self) {
        for conn in self.connections.values() {
            let Some(noq_conn) = conn.handle.upgrade() else {
                continue;
            };
            for (path_id, addr) in &conn.paths {
                if self.state.selected_path.as_ref() == Some(addr) {
                    continue;
                }
                if let Some(path) = noq_conn.path(*path_id) {
                    ...
                    debug!(
                        ...
                        path_challenges_tx = stats.frame_tx.path_challenge,
                        path_responses_tx = stats.frame_tx.path_response,
                        path_challenges_rx = stats.frame_rx.path_challenge,
                        path_responses_rx = stats.frame_rx.path_response,
                        pto_count = stats.pto_count,
                        udp_tx_datagrams = stats.udp_tx.datagrams,
                        udp_rx_datagrams = stats.udp_rx.datagrams,
                        cwnd = stats.cwnd,
                        "probing warm backup path"
                    );
                    if let Err(err) = path.probe_liveness() {
                        trace!(%err, %path_id, ?addr, "failed to probe backup path");
                    }
                }
            }
        }
        ...
        match event {
            NoqPathEvent::Established { id: path_id, .. } => {
                ...
                if let Some(network_path) = self
                    ...
                    && self
                        .state
                        .recovery_probation_until
                        .contains_key(&network_path.remote())
                {
                    self.state.recovery_probation_until.insert(
                        network_path.remote().clone(),
                        Instant::now() + self.state.path_recovery_probation,
                    );
                }
                self.select_path();
            }
    ...
    fn select_path(&mut self) {
        ...
        let mut selected_addr = {
            let ctx = PathSelectionContext::new(current_path, &self.connections);
            self.state.path_selector.select(&ctx).selected().cloned()
        };

        if let Some(candidate) = selected_addr.as_ref()
            && Some(candidate) != current_path
        {
            let now = Instant::now();
            let current_is_open = current_path.is_some_and(|current| {
                self.connections
                    .values()
                    .any(|conn| conn.paths.values().any(|path| path == current))
            });
            let candidate_is_proving = self
                .state
                .recovery_probation_until
                .get(&candidate.remote())
                .is_some_and(|until| *until > now);
            if current_is_open && candidate_is_proving {
                trace!(%candidate, "recovered path still in failback probation");
                selected_addr = None;
            }
        }
            ...
            && self.state.selected_path.as_ref() != Some(&addr)
        {
            ...
            self.state.recovery_probation_until.remove(&addr.remote());
            event!(
    ...
    /// - Keeps validated non-selected paths warm as [`PathStatus::Backup`]
    ...
    /// Reopens known addresses that no longer have a live QUIC path.
    ///
    /// The address remains in [`RemotePathState`] after an abandoned path, so periodically
    /// attempting it provides interface/ISP recovery without requiring a daemon restart or
    /// a platform-specific network-change notification.  `open_path_on_conn` is idempotent
    /// for paths that are open or still validating.
    fn reopen_known_paths(&mut self) {
        ...
        candidates.truncate(MAX_MULTIPATH_PATHS as usize);

        for candidate in candidates {
            let missing_from_any_connection = self.connections.values().any(|conn| {
                conn.handle.upgrade().is_some()
                    && !conn
                        .paths
                        .values()
                        .any(|path| path.remote() == candidate.remote())
                    && !conn
                        .opening_paths
                        .values()
                        .any(|path| path.remote() == candidate.remote())
            });
            if !missing_from_any_connection {
                continue;
            }
            self.state.recovery_probation_until.insert(
                candidate.remote().clone(),
                Instant::now() + self.state.path_recovery_probation,
            );
            self.open_path_on_all_conns(&candidate);
        }
    }
...
impl State {
    ...
    fn open_path_on_conn(
        ...
        if conn_state
            .paths
            .values()
            .any(|a| a.remote() == open_addr.remote())
            || conn_state
                .opening_paths
                .values()
                .any(|a| a.remote() == open_addr.remote())
        ...
        match fut.path_id() {
            Some(path_id) => {
                conn_state.opening_paths.insert(path_id, open_addr.clone());
...
struct ConnectionState {
    ...
    /// Paths opened in noq but still awaiting validation/establishment.
    opening_paths: FxHashMap<PathId, transports::FourTuple>,
...
impl ConnectionState {
    ...
    fn add_open_path(
        ...
        self.opening_paths.remove(&path_id);
        match network_path {
...
impl<'a> PathSelectionData<'a> {
    ...
    /// Stable identity for tracking health of one live QUIC path across selector calls.
    #[cfg_attr(not(feature = "unstable-custom-transports"), allow(dead_code))]
    pub fn health_key(&self) -> Option<(usize, PathId)> {
        match &self.source {
            StatsSource::Live { path_id, conn } => Some((conn.stable_id(), *path_id)),
            #[cfg(test)]
