...
use noq::{NetworkChangeHint, TokenStore, crypto::rustls::QuicServerConfig};
use rand::RngExt;
use tokio::sync::{
    Mutex as AsyncMutex,
    mpsc::{self},
...
///
/// Path abandonment is deliberately slower than traffic failover.  QUIC PTO health moves
/// traffic to a warm backup quickly, while this longer lifetime lets the original path
/// recover in place without consuming path IDs/CIDs or tearing down the connection during
/// ordinary Wi-Fi/cellular interruptions.
...
pub(crate) struct Options {
    ...
    pub(crate) path_recovery_interval: Duration,
    pub(crate) path_recovery_probation: Duration,
...
impl StaticConfig {
    ...
    pub(crate) fn create_client_config(
        ...
        session_partition: Option<&crate::endpoint::TlsSessionPartition>,
    ) -> noq_proto::ClientConfig {
        let mut quic_client_config = self
            .tls_config
            .make_client_config_for_peer(expected_peer, session_partition, self.keylog)
            .expect("validated endpoint TLS configuration remains valid");
            ...
            fn is_path_recoverable(
                ...
                match MultipathMappedAddr::from(network_path.remote()) {
                    ...
                    MultipathMappedAddr::Ip(_) => {
                        // A multipath connection owns independently validated IP paths.
                        // A generic interface snapshot must not abandon all of them when
                        // one interface changes: snapshots can be transiently incomplete,
                        // and closing every PathId destroys the warm backup that is meant
                        // to carry the connection. Keep IP paths recoverable here; QUIC PTO
                        // health switches traffic and the per-path idle timeout retires a
                        // path only after it has actually stopped responding.
                        true
                    }
...
mod tests {
    ...
    async fn socket_connect_with_transport_config(
        ...
        .make_client_config_for_peer(endpoint_id, None, true)?;
        ...
        let connect = ep
            .connect_with(
                ...
                tls::DEFAULT_VISIBLE_SERVER_NAME,
            )
            .std_context("connect")?;
