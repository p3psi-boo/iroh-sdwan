...
impl<T: ConnectionState> Connection<T> {
    ...
    /// Waits for one application datagram, then drains the current buffered
    /// burst while holding the underlying QUIC state once.
    #[inline]
    pub fn read_datagram_batch(
        &self,
        maximum: usize,
    ) -> impl Future<Output = Result<Vec<Bytes>, ConnectionError>> + '_ {
        self.inner.read_datagram_batch(maximum)
    }

    /// Drains up to `maximum` application datagrams that are already buffered
    /// without registering another receive wakeup.
    #[inline]
    pub fn try_read_datagram_batch(&self, maximum: usize) -> Result<Vec<Bytes>, ConnectionError> {
        self.inner.try_read_datagram_batch(maximum)
    }
    ...
    /// Transmits a bounded batch of application datagrams while amortizing the
    /// QUIC connection-state lock across the admitted prefix.
    #[inline]
    pub fn send_datagram_batch_wait(
        &self,
        datagrams: Vec<bytes::Bytes>,
    ) -> impl Future<Output = Result<(), SendDatagramError>> + '_ {
        self.inner.send_datagram_batch_wait(datagrams)
    }

    /// Submit a bounded DATAGRAM batch through the connection driver's
    /// producer mailbox without taking the protocol-state mutex here.
    #[inline]
    pub async fn send_datagram_batch_buffered(
        &self,
        datagrams: Vec<bytes::Bytes>,
    ) -> Result<(), SendDatagramError> {
        self.inner.send_datagram_batch_buffered(datagrams).await
    }

    /// Submit a bounded batch of contiguous or two-range DATAGRAM payloads
    /// without coalescing split payloads before final QUIC packetization.
    #[inline]
    pub async fn send_datagram_payload_batch_buffered(
        &self,
        datagrams: Vec<super::DatagramPayload>,
    ) -> Result<(), SendDatagramError> {
        self.inner
            .send_datagram_payload_batch_buffered(datagrams)
            .await
    }

    /// Submit a strict-priority DATAGRAM batch ahead of ordinary staged and
    /// protocol-buffered DATAGRAMs on this QUIC connection.
    pub async fn send_datagram_batch_buffered_priority(
        &self,
        datagrams: Vec<bytes::Bytes>,
    ) -> Result<(), SendDatagramError> {
        self.inner
            .send_datagram_batch_buffered_priority(datagrams)
            .await
    }

    /// Submit a strict-priority batch of contiguous or split DATAGRAM
    /// payloads ahead of ordinary staged traffic.
    pub async fn send_datagram_payload_batch_buffered_priority(
        &self,
        datagrams: Vec<super::DatagramPayload>,
    ) -> Result<(), SendDatagramError> {
        self.inner
            .send_datagram_payload_batch_buffered_priority(datagrams)
            .await
    }
    ...
    /// Set the native connection-local QUIC DATAGRAM byte limit.
    #[inline]
    pub fn set_datagram_send_buffer_limit(&self, value: usize) -> usize {
        self.inner.set_datagram_send_buffer_limit(value)
    }

    /// Current native connection-local QUIC DATAGRAM byte limit.
    #[inline]
    pub fn datagram_send_buffer_limit(&self) -> usize {
        self.inner.datagram_send_buffer_limit()
    }
    ...
    pub fn congestion_tunables(&self, path_id: PathId) -> Option<Arc<dyn Any + Send + Sync>> {
        self.inner.congestion_tunables(path_id)
    }
...
mod tests {
    ...
    #[tokio::test]
    #[traced_test]
    async fn test_0rtt_disabled() -> Result {
        ...
        connect_client_0rtt_expect_err(&client, server.addr()).await?;
        // A resumed handshake must still not expose an early-data connection.
        connect_client_0rtt_expect_err(&client, server.addr()).await?;
    ...
    async fn test_0rtt_after_server_restart() -> Result {
        ...
        let server =
            spawn_0rtt_server(server_key.clone(), info_span!("server-initial"), true).await?;
        ...
        let server = spawn_0rtt_server(server_key, info_span!("server-restart"), true).await?;
