...
pub use noq::{
    ...
    DatagramPayload,      // iroh::endpoint::Connection
    Dir,                  // noq::StreamId
...
pub use noq_proto::{
    ...
    congestion::{
        Bbr3Tunables,       // iroh::endpoint::Connection runtime tuning
        ...
        ControllerSnapshot, // noq_proto::congestion::ControllerMetrics
    },
    crypto::{
...
impl QuicTransportConfigBuilder {
    ...
    /// Configures an outbound rate ceiling in bytes per second for each QUIC
    /// connection. `None` leaves rate selection entirely to congestion
    /// control.
    pub fn max_outgoing_bytes_per_second(mut self, value: Option<u64>) -> Self {
        self.0.max_outgoing_bytes_per_second(value);
        self
    }
    ...
    ///
    /// The caller is responsible for keeping this consistent with the
    /// connection idle timeout. Ironet validates its runtime configuration
    ...
    ///
    /// The caller is responsible for selecting an interval below the path and
