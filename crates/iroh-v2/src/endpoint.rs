...
use std::{collections::BTreeSet, net::SocketAddr, pin::Pin, sync::Arc, time::Duration};
...
pub mod presets;
pub(crate) mod quic;
pub use quic::{Bbr3Tunables, ControllerSnapshot};
...
pub use self::{
    ...
    quic::{
        ...
        ControllerMetrics, CryptoError, DatagramPayload, DecryptedInitial, Dir,
        ExportKeyingMaterialError, FrameStats, FrameType, HandshakeTokenKey, HeaderKey,
        IdleTimeout, IncomingAlpns, Keys, MtuDiscoveryConfig, OpenBi, OpenUni, PacketKey, PathId,
...
impl Builder {
    ...
    pub fn empty() -> Self {
        ...
        Self {
            ...
            path_recovery_interval: Duration::from_secs(1),
            path_recovery_probation: Duration::from_secs(2),
    ...
    pub async fn bind(self) -> Result<Endpoint, BindError> {
        ...
        let sock_opts = socket::Options {
            ...
            path_recovery_interval: self.path_recovery_interval,
            path_recovery_probation: self.path_recovery_probation,
    ...
    /// Enables or disables TLS 0-RTT early data for native iroh connections.
    ///
    /// Disabling this prevents clients from sending application data before the
    /// handshake completes and makes servers reject early data. TLS session
    /// tickets remain available for handshake resumption. The default is `true`.
    ...
    /// Configures how often validated backup paths receive an exact liveness
    /// probe while they are not carrying application traffic.
    #[cfg(feature = "unstable-custom-transports")]
    pub fn path_recovery_interval(mut self, interval: Duration) -> Self {
        self.path_recovery_interval = interval;
        self
    }

    /// Configures the hold-down applied to a reopened path before failback.
    #[cfg(feature = "unstable-custom-transports")]
    pub fn path_recovery_probation(mut self, probation: Duration) -> Self {
        self.path_recovery_probation = probation;
        self
    }
}
...
impl Endpoint {
    ...
    pub async fn connect_with_opts(
        ...
        let client_config = self.inner.static_config.create_client_config(
            alpn_protocols,
            transport_config.clone(),
            endpoint_id,
            options.tls_session_partition.as_ref(),
        );

        let dest_addr = mapped_addr.private_socket_addr();
        let connect = self.inner.noq_endpoint().connect_with(
            client_config,
            dest_addr,
            &options.visible_server_name,
        )?;
...
pub struct ConnectOptions {
    ...
    visible_server_name: String,
    tls_session_partition: Option<TlsSessionPartition>,
}
...
impl Default for ConnectOptions {
    fn default() -> Self {
        Self {
            ...
            visible_server_name: tls::DEFAULT_VISIBLE_SERVER_NAME.to_owned(),
            tls_session_partition: None,
        }
    }
}

/// Application-level dimensions that isolate QUIC TLS resumption tickets.
///
/// The peer identity is always added internally. This structure supplies the
/// remaining V2 dimensions so peers sharing one visible SNI never share ticket
/// state across networks, cover generations, or QUIC versions.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct TlsSessionPartition {
    pub(crate) network_id: String,
    pub(crate) cover_profile: u32,
    pub(crate) quic_version: u32,
}

impl TlsSessionPartition {
    /// Creates a ticket partition for one logical application session domain.
    pub fn new(network_id: impl Into<String>, cover_profile: u32, quic_version: u32) -> Self {
        Self {
...
impl ConnectOptions {
    ...
    /// Overrides only the DNS name sent in the TLS ClientHello. Peer
    /// authentication remains bound to the [`EndpointId`] supplied in the
    /// [`EndpointAddr`], so this name is not used as an identity source.
    pub fn with_visible_server_name(mut self, server_name: impl Into<String>) -> Self {
        self.visible_server_name = server_name.into();
        self
    }

    /// Isolates TLS resumption state by network, cover generation, and QUIC
    /// version in addition to the authenticated EndpointId.
    pub fn with_tls_session_partition(mut self, partition: TlsSessionPartition) -> Self {
        self.tls_session_partition = Some(partition);
        self
    }
}
...
mod tests {
    ...
    async fn endpoint_two_direct_add_relay() -> Result {
        ...
        async fn connect(
            ...
            // Initial selection is deliberately race-free but not ordered by
            // transport kind: a local relay can finish the first handshake
            // before direct-address validation. Assert the V2-relevant
            // invariant instead—one logical connection discovers and retains
            // both paths without reconnecting or changing peer identity.
            let mut paths = conn.paths_stream();
            let both_paths = time::timeout(Duration::from_secs(5), async move {
                while let Some(infos) = paths.next().await {
                    info!(?infos, "new PathInfos");
                    if infos.iter().any(|info| info.is_ip())
                        && infos.iter().any(|info| info.is_relay())
                    {
                        info!("client has direct and relay paths");
                        return true;
                    }
                }
                false
            ...
            assert!(
                both_paths,
                "connection did not discover both IP and relay paths"
            );
