...
pub(super) struct PacketNumberSpace {
    ...
    /// Whether the next path-liveness packet may exceed the congestion window.
    ///
    /// This is deliberately separate from `loss_probes`: an explicit
    /// PING/PATH_CHALLENGE or PATH_RESPONSE must not steal connection-global
    /// retransmits from an in-flight packet on another multipath path.
    pub(super) congestion_exempt_probe: bool,
