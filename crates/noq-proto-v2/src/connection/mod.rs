...
impl Connection {
    ...
    pub fn set_path_status(
        ...
        let update = path.status.local_update(status);
        let prev = update.unwrap_or_else(|| path.local_status());
        if prev == PathStatus::Backup && status == PathStatus::Available {
            path.rtt.on_path_activated();
            path.congestion.on_path_activated();
            path.pacing.on_path_activated();
        }
        if update.is_some() {
            self.spaces[SpaceId::Data]
                .pending
    ...
    fn poll_transmit_path_space(
        ...
        loop {
            ...
            let needs_congestion_exempt_probe = self.spaces[space_id]
                .for_path(path_id)
                .congestion_exempt_probe;
            let space_will_send = {
            ...
            if transmit.datagram_remaining_mut() == 0 {
                ...
                // Consume the one-packet exemption only after congestion and
                // pacing checks passed and a datagram will definitely be built.
                self.spaces[space_id]
                    .for_path(path_id)
                    .congestion_exempt_probe = false;
    ...
    fn path_congestion_check(
        ...
        let path_space = self.spaces[space_id].for_path(path_id);
        let congestion_exempt = path_space.loss_probes > 0 || path_space.congestion_exempt_probe;

        if can_send.other && !congestion_exempt && !can_send.close {
    ...
    pub fn path_stats(&mut self, path_id: PathId) -> Option<PathStats> {
        ...
        stats.pto_count = path.data.pto_count;
    ...
    /// Queue a liveness probe that may exceed a path's congestion window.
    ///
    /// This is intended for a validated warm backup whose old keepalives filled its
    /// tiny standby window while the interface was unavailable. It grants exactly one
    /// probe packet and does not make application data congestion-exempt.
    pub fn probe_path_liveness(&mut self, path: PathId) -> Result<(), ClosedPath> {
        let request_immediate_ack = self.peer_supports_ack_frequency();
        let path_space = self.spaces[self.highest_space]
            .number_spaces
            .get_mut(&path)
            .ok_or(ClosedPath { _private: () })?;
        path_space.congestion_exempt_probe = true;
        // Coalesce a PING with PATH_CHALLENGE. PING makes the packet
        // non-probing under RFC 9000, which forces ordinary ACK/path handling
        // even on an already-validated warm path; PATH_CHALLENGE still gives
        // us the stronger exact-four-tuple response when available.
    ...
    pub fn congestion_tunables(&self, path_id: PathId) -> Option<Arc<dyn Any + Send + Sync>> {
        self.path(path_id)?.congestion.tunables()
    }
    ...
    fn inner_on_ack_received(
        ...
        for packet in newly_acked.elts() {
            if let Some(info) = self.spaces[space].for_path(path).take(packet) {
                let path_stats = self.path_stats.get_mut(path);
                path_stats.acked_packets = path_stats.acked_packets.saturating_add(1);
    ...
    fn handle_packet(
        ...
        let result = match decrypted {
            ...
            Ok((packet, pn)) => {
                ...
                let stats = self.path_stats.get_mut(path_id);
                stats.authenticated_rx_packets = stats.authenticated_rx_packets.saturating_add(1);
            ...
            match frame {
                ...
                Frame::PathChallenge(challenge) => {
                    trace!(
                        ...
                        %network_path,
                        %path_id,
                        "received on-path PATH_CHALLENGE"
                    );
                    let path_space = self.spaces[SpaceKind::Data].for_path(path_id);
                    path_space
                        .pending_path_responses
                        .push(number, challenge.0, network_path);
                    // PATH_RESPONSE is the proof used to recover a warm path. It must
                    // not deadlock behind the stale congestion window left by an
                    // outage, so grant exactly one response packet the same bounded
                    // exemption used by a tail-loss probe.
                    path_space.congestion_exempt_probe = true;
                ...
                Frame::PathResponse(response) => {
                    ...
                    {
                        // A successful NAT-traversal challenge is the same
                        // unpredictable request/response proof as ordinary
                        // path validation, and additionally arrived on this
                        // exact local four-tuple. Account it on an already
                        // open path so recovery/failback does not depend on
                        // PATH_ACKs that QUIC multipath may return elsewhere.
                        if let Some(path_id) = self.paths.iter().find_map(|(id, path)| {
                            (path.data.network_path.is_probably_same_path(&network_path)
                                && !self.abandoned_paths.contains(id))
                            .then_some(*id)
                        }) {
                            let stats = self.path_stats.get_mut(path_id);
                            stats.validated_challenges =
                                stats.validated_challenges.saturating_add(1);
                            debug!(
                                %response,
                                %network_path,
                                %path_id,
                                "validated NAT PATH_RESPONSE on open path"
                            );
                        }
    ...
    fn handle_path_response_on_path(
        ...
        receiving_path_id: PathId,
    ) {
        let is_multipath_negotiated = self.is_multipath_negotiated();
        // RFC 9000 permits PATH_RESPONSE on a different network path from the
        // PATH_CHALLENGE. Attribute the response to the path that owns the
        // unpredictable token, rather than the path carrying the response.
        let path_id = self
            .paths
            .iter()
            .find_map(|(id, path)| {
                path.data
                    .has_unconfirmed_challenge(response.0)
                    .then_some(*id)
            })
            .unwrap_or(receiving_path_id);
        let path = self
            .paths
            .get_mut(&path_id)
            .expect("payload is processed only after the path becomes known");
        match path.data.on_path_response_received(now, response.0) {
            paths::OnPathResponseReceived::OnPath if !self.abandoned_paths.contains(&path_id) => {
                debug!(
                    %response,
                    %receiving_path_id,
                    attributed_path_id = %path_id,
                    "validated on-path PATH_RESPONSE"
                );
                let stats = self.path_stats.get_mut(path_id);
                stats.validated_challenges = stats.validated_challenges.saturating_add(1);
                let qlog = self.qlog.with_time(now);
                self.timers.stop(
    ...
    fn can_send_1rtt(&self, path_id: PathId, max_size: usize) -> SendableFrames {
        ...
        let other = self.streams.can_send_stream_data()
            || self
                ...
                .outgoing
                .front()
                .is_some_and(|x| x.datagram.size(true) <= max_size);
