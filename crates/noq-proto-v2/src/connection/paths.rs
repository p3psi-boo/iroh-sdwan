...
impl PathData {
    ...
    pub(super) fn has_unconfirmed_challenge(&self, token: u64) -> bool {
        self.unconfirmed_challenges.contains_key(&token)
    }
    ...
    pub(super) fn record_path_challenge_sent(
        ...
        tracing::trace!(token, %network_path, "recorded on-path PATH_CHALLENGE");
    }
    ...
    pub(super) fn on_path_response_received(
        ...
        match self.unconfirmed_challenges.remove(&token) {
            ...
            Some(info) if self.network_path.is_probably_same_path(&info.network_path) => {
                ...
                let was_validated = std::mem::replace(&mut self.validated, true);
                if !was_validated {
                ...
                // RFC 9002 permits this sample only for initial validation. A
                // delayed response to a warm-path challenge after an outage
                // must not overwrite established RTT and suppress failback.
                if !was_validated {
...
impl RttEstimator {
    ...
    /// Re-seeds the short-term estimator when a warm backup becomes active.
    ///
    /// Sparse recovery probes can spend time behind PTO/pacing and inflate the
    /// smoothed RTT even though their exact four-tuple is reachable again. The
    /// path's historical minimum is a better startup estimate for the first
    /// data burst. Fresh ACK samples immediately adapt it if the route really
    /// changed, while avoiding a stale multi-PTO delay on ordinary failback.
    pub(crate) fn on_path_activated(&mut self) {
        if self.smoothed.is_none() {
            return;
        }
        self.latest = self.min;
        self.smoothed = Some(self.min);
        self.var = self.min / 2;
    }
...
mod tests {
    ...
    #[test]
    fn recovered_path_with_unspecified_local_ip_accepts_original_challenge_tuple() {
        let remote = "[2001:db8::2]:443".parse().unwrap();
        let current = FourTuple::from_remote(remote);
        let challenged = FourTuple::new(remote, Some("2001:db8::1".parse().unwrap()));

        assert!(current.is_probably_same_path(&challenged));
        assert!(!challenged.is_probably_same_path(&current));
    }

    #[test]
    fn activating_warm_path_discards_probe_queue_delay_but_preserves_minimum() {
        let mut rtt = RttEstimator::new(Duration::from_millis(100));
        rtt.update(Duration::ZERO, Duration::from_millis(5));
        rtt.update(Duration::ZERO, Duration::from_millis(205));
        assert!(rtt.get() > Duration::from_millis(5));

        rtt.on_path_activated();

        assert_eq!(rtt.get(), Duration::from_millis(5));
        assert_eq!(rtt.conservative(), Duration::from_millis(5));
        assert_eq!(rtt.min(), Duration::from_millis(5));
    }
}
