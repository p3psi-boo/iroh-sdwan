...
pub struct PathStats {
    ...
    pub cwnd: u64,
    /// Consecutive probe timeouts since the most recent acknowledgement.
    ///
    /// This is a live path-health signal rather than a cumulative counter.  A value of
    /// zero means the path has acknowledged traffic since its last PTO; increasing values
    /// indicate that the path may be black-holed.
    pub pto_count: u32,
    /// Packets sent on this path that have subsequently been acknowledged.
    ///
    /// Unlike received UDP datagrams, this proves bidirectional progress for the
    /// specific sending path even when multipath carries its PATH_ACK elsewhere.
    pub acked_packets: u64,
    /// Packets successfully authenticated and decrypted on this path.
    ///
    /// This excludes arbitrary UDP datagrams that fail QUIC authentication and
    /// therefore provides a trustworthy reverse-direction progress signal.
    pub authenticated_rx_packets: u64,
    /// Successful on-path PATH_CHALLENGE/PATH_RESPONSE exchanges initiated by
    /// this endpoint. Unlike a PATH_ACK, the response proves that the probed
    /// four-tuple carried the challenge in both directions.
    pub validated_challenges: u64,
...
impl std::ops::Add<PathStats> for ConnectionStats {
    ...
    fn add(self, rhs: PathStats) -> Self::Output {
        ...
        let PathStats {
            ...
            pto_count: _,
            acked_packets: _,
            authenticated_rx_packets: _,
            validated_challenges: _,
            congestion_events: _,
