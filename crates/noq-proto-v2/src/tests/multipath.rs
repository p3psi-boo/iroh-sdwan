...
#[test]
fn warm_backup_liveness_probe_records_validated_challenge() -> TestResult {
    let _guard = subscribe();
    let mut pair = ConnPair::builder()
        .enable_multipath()
        .disable_mtud_discovery()
    ...
    pair.drive();
    assert_eq!(
        pair.conn_mut(Client).path_status(path_1)?,
        PathStatus::Available
    );

    let before = pair.path_stats(Client, PathId::ZERO).unwrap();
    pair.conn_mut(Client).probe_path_liveness(PathId::ZERO)?;
    pair.drive();
    let after = pair.path_stats(Client, PathId::ZERO).unwrap();

    assert_eq!(after.validated_challenges, before.validated_challenges + 1);
    assert!(after.frame_tx.path_challenge > before.frame_tx.path_challenge);
    assert!(after.frame_tx.ping > before.frame_tx.ping);
    Ok(())
}
