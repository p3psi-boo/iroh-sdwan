...
/// An outgoing unreliable DATAGRAM payload.
///
/// Keeping the payload as two immutable byte ranges lets applications prepend
/// a small protocol header without first copying the large body into an
/// intermediate contiguous allocation. QUIC packet protection still writes
/// both ranges into the final contiguous packet buffer before encryption, so
/// this changes neither wire format nor AEAD semantics.
...
impl DatagramPayload {
    /// Construct a contiguous outgoing DATAGRAM payload.
    pub fn contiguous(data: Bytes) -> Self {
        Self {
            head: data,
            tail: Bytes::new(),
        }
    ...
    /// Construct an outgoing DATAGRAM payload from two adjacent wire ranges.
    pub fn split(head: Bytes, tail: Bytes) -> Self {
        Self { head, tail }
    }

    /// Total payload length across both ranges.
    ...
    /// Return the two adjacent wire ranges.
...
impl fmt::Display for DatagramPayload {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(formatter, "DATAGRAM len: {}", self.len())
    }
}

impl FrameStruct for DatagramPayload {
    const SIZE_BOUND: usize = 1 + 8;
}

impl Encodable for DatagramPayload {
    fn encode<B: BufMut>(&self, out: &mut B) {
        ...
        out.write(VarInt::from_u64(self.len() as u64).unwrap());
