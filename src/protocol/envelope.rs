...
impl Envelope {
    ...
    pub fn encode(&self) -> Result<Bytes> {
        let header_len = HEADER_LEN + self.extension.len();
            ...
            "v4 envelope extension is too large"
        );
        ensure!(
            header_len <= u16::MAX as usize,
            "v4 envelope header is too large"
        );
    ...
    pub fn decode(bytes: Bytes) -> Result<Self> {
        ensure!(bytes.len() >= HEADER_LEN, "truncated v4 envelope");
        ensure!(&bytes[..4] == MAGIC, "invalid v4 envelope magic");
        let raw_kind = u16::from_be_bytes(bytes[4..6].try_into().unwrap());
        let kind = MessageType::from_wire(raw_kind)
            .ok_or_else(|| anyhow::anyhow!("unknown v4 message type {raw_kind}"))?;
        ...
        let header_len = usize::from(u16::from_be_bytes(bytes[8..10].try_into().unwrap()));
            ...
            bytes[10..12] == [0, 0],
            "unsupported v4 envelope reserved bits"
        );
        ensure!(
            (HEADER_LEN..=MAX_HEADER_LEN).contains(&header_len) && header_len <= bytes.len(),
            "invalid v4 envelope header length"
        );
        Ok(Self {
            kind,
            flags,
